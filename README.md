...
└── data/
    ├── employees.csv          # Данные авиакомпаний
    ├── trips.xlsx           # Данные рейсов
    └── allowances.json           # Данные билетов
```
...
- Столбчатая диаграмма общих командировочных расходов по отделам
- Круговая диаграмма долей отделов в общем бюджете на командировки
- Сравнительный анализ: количество командировок, общие дни в поездках и средние расходы на сотрудника по отделам


##  Результаты

Все результаты сохраняются в папке `data/`:
- consolidated_department_data.csv — консолидированные данные по каждой командировке (сотрудник, отдел, город, дни, суточные, расходы)
